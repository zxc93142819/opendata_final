...
def get_restaurant_now(user_id):
    ...
    with open(file_path , encoding="utf-8") as f:
        data = json.load(f)
    random_numbers = random.sample(range(0, 30), 10)
    spe_search_imageurl = []
    # 以後做map 查詢距離
    # spe_search_website = []
    spe_search_address = []
    ...
    for i in random_numbers :
        ...
        # 輸出轉換後的地址
        spe_search_name.append(data[i]["ScenicSpotName"])
        spe_search_address.append("地址啦")
        spe_search_imageurl.append(data[i]["Picture"]["PictureUrl1"])
        # spe_search_detail.append(data[i]["DescriptionDetail"])
